...
describe('FileAssetTestPromise.test.js', async function () {
    ...
    it('SUB_MEDIA_MEDIALIBRARY_OPENANDCLOSE_ASSET_PROMISE_005_01', 0, async function (done) {
        ...
        try {
            ...
            expect(res.bytesRead == assetProps.bytesRead).assertTrue();
            asset1 = dataList[1];
            ...
            expect(write == assetProps.write).assertTrue();

            await asset.close(fd);
            await asset1.close(fd1);

            if (isNum(fd) && res.bytesRead == assetProps.bytesRead && write == assetProps.write) {
    ...
    it('SUB_MEDIA_MEDIALIBRARY_OPENANDCLOSE_ASSET_PROMISE_005_10', 0, async function (done) {
        ...
        try {
            ...
            let fileFetchOp = {
                selections: fileKeyObj.MEDIA_TYPE + '= ? AND ' + fileKeyObj.RELATIVE_PATH + '= ? ',
    ...
    it('SUB_MEDIA_MEDIALIBRARY_OPENANDCLOSE_ASSET_PROMISE_005_34', 0, async function (done) {
        ...
        try {
            ...
            fd1 = await asset.open('r');
            await asset.close(fd1);
            console.info('MediaLibraryTest : SUB_MEDIA_MEDIALIBRARY_OPENANDCLOSE_ASSET_PROMISE_005_34 fd:' + fd);
            console.info('MediaLibraryTest : SUB_MEDIA_MEDIALIBRARY_OPENANDCLOSE_ASSET_PROMISE_005_34 fd:' + fd1);


            if (isNum(fd) && isNum(fd1)) {
